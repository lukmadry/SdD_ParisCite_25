...
lm(m ~ x)
```
** Est-ce qu'on peut prévoir la difference en `x` en fonction de taille d'un bruit ? Similaire au calcul de covariance, etudier l'influence de $\sigma$, c'est-a-dire- une parametre qui va remplacer `0.1` dans le code au-dessus, sur les coefficients `a,b`.
