---
title: "TP2 - le semaine du 20 janvier"
author: "Łukasz Mądry"
date: "2025-01-21"
...
* Pour $n \in \mathbb{N}$ fixe, calculer le factoriel $n!$
...
Finalement on va etudier les matrices et tableaux de données. On peut créer une matrice avec un commande `matrix`. 
...
### Plus d'exercices au cas ou vous avez fini avant l'heure

Refléchissez aux questions suivantes. Le mieux serait d'essayer de donner la reponse avant que vous lanciez un code R.

* Considerons `y<-5` et `y <- 5`, `y < -5`. Auquel comportement vous vous attendez ? Verifiez.

Soit `x` un vecteur numerique.
* Qu'est ce qu'est x[c()]? Est-ce la meme chose que x[]?
• Est-ce x[c(1, 1, 1)] equivalent a x[1]?
• Pareil pour x[1] et x["1"]?
• x[c(-1, -1, -1)] et x[-1]?
• Que'est ce que fait x[c(0, 1, 2, NA)]  ?
• Le resultat de x[0] ?
• De meme pour x[1, 2, 3] ?
